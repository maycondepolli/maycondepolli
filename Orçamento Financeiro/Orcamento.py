...
# ==============================================================================
# --- FUNÇÃO MELHORADA PARA FORMATAR DATA AUTOMATICAMENTE ---
# ==============================================================================
def formatar_data_entrada(event):
    """Insere automaticamente as barras '/' na digitação da data e limita o tamanho"""
    # Ignora teclas de controle como Backspace, Delete e Setas
    ...
    # Limita o tamanho máximo para 8 dígitos (DDMMAAAA)
...
# ==============================================================================
# --- FUNÇÃO MELHORADA PARA FORMATAR VALOR AUTOMATICAMENTE ---
# ==============================================================================
def formatar_valor_entrada(event):
    """Formata automaticamente o valor com vírgula decimal e pontos de milhar"""
    # Ignora teclas de controle
    if event.keysym in ("Backspace", "Delete", "Left", "Right", "Tab", "Return"):
        return
        
    entry_valor = event.widget
    ...
    valor_formatado = f"R$ {reais_formatado},{centavos:02d}"
    
    # Atualiza o campo
    if texto != valor_formatado:
        entry_valor.delete(0, tk.END)
        entry_valor.insert(0, valor_formatado)

# ==============================================================================
# --- FUNÇÃO PARA CONVERTER VALOR FORMATADO PARA FLOAT ---
# ==============================================================================
def converter_valor_para_float(valor_formatado):
    """Converte um valor formatado (R$ 1.500,00) para float"""
    try:
        # Remove "R$ " se existir
        valor_limpo = valor_formatado.replace("R$ ", "").strip()
...
# ==============================================================================
# --- FUNÇÃO PARA CONVERTER FLOAT PARA VALOR FORMATADO ---
# ==============================================================================
def converter_float_para_valor_formatado(valor_float):
    """Converte um float para string formatada (R$ 1.500,00)"""
    try:
                ...
                for linha in f:
                    ...
                    if len(partes) == 5:
                        ...
                        if incluir:
                            ...
                            valor_num = converter_valor_para_float(valor_formatado)
                            total_ent += valor_num
...
# ==============================================================================
# --- FUNÇÃO MELHORADA PARA EXPORTAR PARA EXCEL ---
# ==============================================================================
def exportar_para_excel():
    """Exporta todos os dados filtrados para uma planilha Excel com formatação correta"""
    ...
    try:
        ...
        # Adiciona dados com formatação correta
        for valores in entradas_filtradas:
            data, pago, categoria, valor_formatado, descricao = valores
            # Converte valor para float e depois formata para Excel
            valor_float = converter_valor_para_float(valor_formatado)
            ws_entradas.append([data, pago, categoria, valor_float, descricao])
        
        # Formata a coluna de valores como moeda
        for row in range(2, ws_entradas.max_row + 1):
            celula = ws_entradas.cell(row=row, column=4)
        ...
        ws_entradas.cell(row=ultima_linha, column=4, value=total_ent)
        ws_entradas.cell(row=ultima_linha, column=4).number_format = 'R$ #,##0.00'
        ...
        ws_entradas.cell(row=ultima_linha, column=3, value="TOTAL:")
        ws_entradas.cell(row=ultima_linha, column=3).font = Font(bold=True)
        ...
        for valores in saidas_filtradas:
            data, pago, categoria, valor_formatado, descricao = valores
            valor_float = converter_valor_para_float(valor_formatado)
            ws_saidas.append([data, pago, categoria, valor_float, descricao])
        
        # Formata a coluna de valores como moeda
        for row in range(2, ws_saidas.max_row + 1):
            celula = ws_saidas.cell(row=row, column=4)
            celula.number_format = 'R$ #,##0.00'
        ...
        ws_saidas.cell(row=ultima_linha, column=4, value=total_sai)
        ws_saidas.cell(row=ultima_linha, column=4).number_format = 'R$ #,##0.00'
        ...
        ws_saidas.cell(row=ultima_linha, column=3, value="TOTAL:")
        ws_saidas.cell(row=ultima_linha, column=3).font = Font(bold=True)
        ...
        ws_resumo.cell(row=linha_atual, column=2, value=total_ent)
        ws_resumo.cell(row=linha_atual, column=2).number_format = 'R$ #,##0.00'
        ...
        ws_resumo.cell(row=linha_atual, column=1).font = Font(bold=True)
        ws_resumo.cell(row=linha_atual, column=2, value=total_sai)
        ws_resumo.cell(row=linha_atual, column=2).number_format = 'R$ #,##0.00'
        ...
        ws_resumo.cell(row=linha_atual, column=1).font = Font(bold=True)
        ws_resumo.cell(row=linha_atual, column=2, value=saldo)
        ws_resumo.cell(row=linha_atual, column=2).number_format = 'R$ #,##0.00'
...
def atualizar_saldo_aba4():
    ...
    label_valor_saldo.config(text=converter_float_para_valor_formatado(saldo_atual))

# ==============================================================================
# --- FUNÇÃO MELHORADA PARA CARREGAR DADOS SALVOS ---
# ==============================================================================
def carregar_dados_salvos():
    ...
    if os.path.exists(ARQUIVO_ENTRADAS):
        try:
            ...
            label_soma_entradas.config(text=f"Total de Entradas: {converter_float_para_valor_formatado(total_entradas_num)}")
        except Exception as e:
    ...
    if os.path.exists(ARQUIVO_SAIDAS):
        try:
            ...
            label_soma_saidas.config(text=f"Total de Saídas: {converter_float_para_valor_formatado(total_saidas_num)}")
        except Exception as e:
...
# ==============================================================================
# --- FUNÇÃO DE AÇÃO DO BOTÃO SALVAR (CORRIGIDA) ---
# ==============================================================================
def salvar_dados():
    global total_entradas_num, total_saidas_num
    
    # 1. Coleta as informações digitadas
    ...
    # 2. Converte o valor formatado para float
    try:
        # Remove "R$ " se existir
        ...
        messagebox.showerror("Erro", "Valor inválido! Use formato como R$ 1.500,00 ou 1500,00")
        return
        
    # Formata o valor para salvar no arquivo
    valor_formatado = f"{valor_num:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")

    # 3. Determina se é entrada ou saída
    if pago == "":  # Campo vazio - ENTRADA
    ...
    # 4. Atualiza o saldo geral
    atualizar_saldo_aba4()

    # 5. Limpa os campos
    entrada_data.delete(0, tk.END)
    entrada_data.insert(0, "DD/MM/AAAA")  # Placeholder
    entrada_pago.delete(0, tk.END)
    entrada_categoria.delete(0, tk.END)
    entrada_valor.delete(0, tk.END)
    entrada_valor.insert(0, "R$ 0,00")  # Valor padrão
...
entrada_data.bind("<KeyRelease>", formatar_data_entrada)
entrada_data.insert(0, "DD/MM/AAAA")  # Placeholder
entrada_data.bind("<FocusIn>", lambda e: entrada_data.delete(0, tk.END) if entrada_data.get() == "DD/MM/AAAA" else None)
entrada_data.bind("<FocusOut>", lambda e: entrada_data.insert(0, "DD/MM/AAAA") if entrada_data.get() == "" else None)
...
entrada_valor.insert(0, "R$ 0,00")  # Valor padrão
entrada_valor.bind("<FocusIn>", lambda e: entrada_valor.delete(0, tk.END) if entrada_valor.get() == "R$ 0,00" else None)
entrada_valor.bind("<KeyRelease>", formatar_valor_entrada)
